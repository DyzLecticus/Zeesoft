...
public class XMLFile extends FileObject {
	 ...
	 * @param xml The XML
	 */
	public String parseXML(StringBuffer xml) { 
		if (builder==null) {
			builder = XMLConfig.getInstance().getNewBuilder();
		}
		String errMsg = "";
	...
	private void parseXMLElem(Element pElem, XMLElem parent, String parentPath) throws Exception {
		...
		if (pElem==null) {
			...
			if (pElem.getAttributes()!=null) {
				newXMLElem.setAttributes(pElem.getAttributes());
			}
			...
			if (nod.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (nodElem!=null) { 
					...
					if ((valNode!=null) && (valNode.getNodeValue()!=null)) { 
						nodValue = new StringBuffer(valNode.getNodeValue());
					}
				}
				...
				if (nod.getAttributes()!=null) {
					newXMLElem.setAttributes(nod.getAttributes());
				}
	...
	public StringBuffer toStringBuffer() {
		StringBuffer s = rootElement.toString(false,null,null);
		s.insert(0,createHeader());
		s.append("\n");
		return s;
	}
	
	public StringBuffer toStringReadFormat() {
		StringBuffer s = rootElement.toString(true,null,null);
		s.insert(0,"\r\n");
		s.insert(0,createHeader());
		s.append("\r\n");
		return s;
	...
	public StringBuffer toStringCompressed() {
		...
		StringBuffer c = compressed.toString(false,tagNames,numerics);
