...
public class JsonZSCStateHandler extends JsonHandlerObject {
	public final static String	PATH	= "/state.json"; 
	
	public JsonZSCStateHandler(Config config, ModObject mod) {
					...
					if (cs==null) {
						...
						if (confsElem!=null) {
							for (JsElem confElem: confsElem.children) {
								...
								if (name.length()>0) {
									Confabulator conf = cs.getConfabulator(name);
									String state = "Initializing ...";
									if (conf!=null && conf.getContext("")!=null) {
										Context ctxt = conf.getContext("");
										if (ctxt!=null) {
