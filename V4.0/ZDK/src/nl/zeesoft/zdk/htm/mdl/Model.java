...
public class Model {
	...
	public Model copyTo(Model copy,boolean includeProximalDendrites,boolean includeCells) {
		...
		for (Column column: columns) {
			...
			copy.addColumn(columnCopy,includeProximalDendrites,includeCells);
		}
	...
	protected void addColumn(Column column,boolean includeProximalDendrites,boolean includeCells) {
		columns.add(column);
		putObject(column);
		ColumnGroup groupCopy = columnGroupsById.get(column.columnGroup.getId());
		column.columnGroup = groupCopy;
		if (includeProximalDendrites) {
			putObject(column.proximalDendrite);
	...
	protected String putObject(ModelObject modelObject) {
	...
	protected void removeObject(ModelObject modelObject) {
		if (modelObject instanceof Column) {
