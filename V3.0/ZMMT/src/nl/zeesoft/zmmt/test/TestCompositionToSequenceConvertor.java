...
public class TestCompositionToSequenceConvertor extends TestObject {
	...
	protected void describe() {
		...
		System.out.println("The output of this test shows the time it takes to convert a simple composition.  ");
	}
	...
	protected void test(String[] args) {
		...
		if (seq!=null) {
			...
			if (seq.getTracks().length>=2) {
				...
				assertEqual(seq.getTracks()[1].size(),1412,"Number of track 1 events does not meet expectation");
			}
		}
		System.out.println("Composition to sequence conversion took " + ((new Date()).getTime() - now) + " ms");
	}
}
