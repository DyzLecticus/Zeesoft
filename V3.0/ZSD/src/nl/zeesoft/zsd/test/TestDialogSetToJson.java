...
public class TestDialogSetToJson extends TestEntityToJson {
	...
	protected void test(String[] args) {
		...
		if (json.rootElement.children.size()>0) {
			...
			assertEqual(json.rootElement.children.get(0).children.size(),5,"The number of dialog elements does not match expectation");
			showJsonSample(json);
		}
	...
	protected void testDialogSetContent(DialogSet ds, String suffix) {
		assertEqual(ds.getDialogs().size(),10,"The number of dialogs does not match expectation" + suffix);
